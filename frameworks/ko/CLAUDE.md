...
이 프로젝트는 **AI-Human Cowork 프레임워크** 기반으로 Claude Code와 함께 개발된다.
이 파일은 Claude Code가 세션 시작 시 참조하는 **프로젝트 컨텍스트 파일**이다.
...
1. 이 파일(`CLAUDE.md`)을 읽고 프로젝트 컨텍스트와 협업 규칙을 파악한다.
...
Claude Code는 다음 행동을 **별도 허락 없이 자동으로** 수행한다:
...
| 컨텍스트 변경 | 이 파일(`CLAUDE.md`) 자동 업데이트 |
...
claude
 ...
 # CLAUDE.md를 명시적으로 읽고 시작 (권장)
claude "CLAUDE.md를 읽고 현재 프로젝트 상태에서 이번 세션을 시작해줘"
```
...
```bash
 # 반복 승인 없이 자동 실행 (신뢰할 수 있는 프로젝트에서만 사용)
...
```text
 # 세션 시작 시 권장 프롬프트
CLAUDE.md를 읽고 현재 프로젝트 상태에서 이어서 진행해줘.
TASK-003 이어서 하자.
```
