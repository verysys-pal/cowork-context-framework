...
This project is developed with Claude Code on top of the **AI-Human Cowork Framework**.
This file is the **project context entrypoint** that Claude Code reads at session start.

> GitHub Copilot users should read `.github/copilot-instructions.md`.
...
1. Read this file (`CLAUDE.md`) and understand the project context and collaboration rules.
...
Claude Code automatically performs the following without separate permission:
...
| Trigger | Claude Code Automatic Action |
|--------|-------------------------------|
...
| Context change | Automatically update this file (`CLAUDE.md`) |
...
# Recommended: explicitly point Claude at the framework entrypoint
claude "Read CLAUDE.md and start this session from the current project state"
```
...
```bash
# Enable automatic approval in a trusted project
...
```text
Read CLAUDE.md and resume from the current project state.
...
> Fill the items below when the project starts. Claude Code automatically updates them when they change.
